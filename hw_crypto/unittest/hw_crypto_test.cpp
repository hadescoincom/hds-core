...
extern "C"
{
	...
	void HdsCrypto_KeyKeeper_ReadSlot(uint32_t iSlot, HdsCrypto_UintBig* p)
	{
		memcpy(p->m_pVal, g_pHwEmuNonces->m_pSlot[iSlot].m_pData, HdsCrypto_nBytes);
	}


	void HdsCrypto_KeyKeeper_RegenerateSlot(uint32_t iSlot)
	{
...
HdsCrypto_UintBig& Ecc2BC(const ECC::uintBig& x)
{
	static_assert(sizeof(x) == sizeof(HdsCrypto_UintBig));
	return (HdsCrypto_UintBig&) x;
}

HdsCrypto_CompactPoint& Ecc2BC(const ECC::Point& x)
{
	static_assert(sizeof(x) == sizeof(HdsCrypto_CompactPoint));
	return (HdsCrypto_CompactPoint&) x;
}

void HdsCrypto_InitGenSecure(HdsCrypto_MultiMac_Secure& x, const ECC::Point::Native& ptVal, const ECC::Point::Native& nums)
{
	...
	for (unsigned int i = 0; ; pt += ptVal)
	{
		...
		if (++i == HdsCrypto_MultiMac_Secure_nCount)
			break;
	}

	pt = Zero;
	for (unsigned int iBit = HdsCrypto_nBits; iBit--; )
	{
		pt = pt * ECC::Two;

		if (!(iBit % HdsCrypto_MultiMac_Secure_nBits))
			pt += nums;
	}

	pt = -pt;
	cpc.set_Deferred(Cast::Up<ECC::Point::Compact>(x.m_pPt[HdsCrypto_MultiMac_Secure_nCount]), pt);
	cpc.Flush();
}

void HdsCrypto_InitFast(HdsCrypto_MultiMac_Fast& trg, const ECC::MultiMac::Prepared& p)
{
...
void InitContext()
{
	...
	HdsCrypto_InitGenSecure(pCtx->m_GenG, pt, nums);

	ctx.m_Ipp.J_.m_Fast.m_pPt[0].Assign(pt, true);
	HdsCrypto_InitGenSecure(pCtx->m_GenJ, pt, nums);

	static_assert(ECC::InnerProduct::nDim * 2 == HdsCrypto_MultiMac_Fast_Idx_H);

	for (uint32_t iGen = 0; iGen < ECC::InnerProduct::nDim * 2; iGen++)
		HdsCrypto_InitFast(pCtx->m_pGenFast[iGen], ECC::Context::get().m_Ipp.m_pGen_[0][iGen]);

	HdsCrypto_InitFast(pCtx->m_pGenFast[HdsCrypto_MultiMac_Fast_Idx_H], ECC::Context::get().m_Ipp.H_);
}
...
void TestMultiMac()
{
	...
	uint32_t aa = sizeof(HdsCrypto_MultiMac_Secure);
	uint32_t bb = sizeof(HdsCrypto_MultiMac_Fast);
	uint32_t cc = sizeof(HdsCrypto_MultiMac_WNaf);
	aa;  bb; cc;
	...
	HdsCrypto_MultiMac_WNaf pWnaf[nFast];
	HdsCrypto_MultiMac_Scalar pFastS[nFast];

	HdsCrypto_MultiMac_Secure pGenSecure[nSecure];
	...
	for (uint32_t iGen = 0; iGen < nSecure; iGen++)
	{
		...
		HdsCrypto_InitGenSecure(pGenSecure[iGen], ptVal, nums);
	}
...
void TestNonceGen()
{
	...
	for (int i = 0; i < 3; i++)
	{
		...
		HdsCrypto_NonceGenerator ng2;
		HdsCrypto_NonceGenerator_Init(&ng2, szSalt, sizeof(szSalt), &Ecc2BC(seed));

		for (int j = 0; j < 10; j++)
		{
			ECC::Scalar::Native sk1, sk2;
			ng1 >> sk1;
			HdsCrypto_NonceGenerator_NextScalar(&ng2, &sk2.get_Raw());
	...
	for (int i = 0; i < 3; i++)
	{
		...
		for (int j = 0; j < 4; j++)
		{
			for (int k = 0; k < 3; k++)
			{
				ECC::Scalar::Native sk1, sk2;
				o1 >> sk1;
				HdsCrypto_Oracle_NextScalar(&o2, &sk2.get_Raw());
			...
			o1 << val;
			HdsCrypto_Oracle_Expose(&o2, val.m_pData, val.nBytes);
		}
	}
}

void TestCoin(const CoinID& cid, Key::IKdf& kdf, const HdsCrypto_Kdf& kdf2)
{
	ECC::Hash::Value hv1, hv2;
	cid.get_Hash(hv1);

	HdsCrypto_CoinID cid2;
	...
	HdsCrypto_CoinID_getHash(&cid2, &Ecc2BC(hv2));

	verify_test(hv1 == hv2);

	uint8_t nScheme;
	uint32_t nSubKey;
	bool bChildKdf2 = !!HdsCrypto_CoinID_getSchemeAndSubkey(&cid2, &nScheme, &nSubKey);
	...
	HdsCrypto_CoinID_getSkComm(&kdf2, &cid2, &sk2.get_Raw(), &fp);
	...
	verify_test(HdsCrypto_RangeProof_Calculate(&rp)); // Phase 2
...
void TestCoins()
{
	...
	hkdf.Generate(hv);
	HdsCrypto_Kdf_Init(&kdf2, &Ecc2BC(hv));
...
void TestKdf()
{
	...
	for (int i = 0; i < 3; i++)
	{
		...
		if (i)
		{
			...
			HdsCrypto_Kdf_getChild(&kdf2, iChild, &kdf2);
		}
		else
		{
			hkdf.Generate(hv);
		...
		for (int j = 0; j < 5; j++)
		{
			...
			HdsCrypto_Kdf_Derive_PKey(&kdf2, &Ecc2BC(hv), &sk2.get_Raw());
			verify_test(sk1 == sk2);

			hkdf.DeriveKey(sk1, hv);
			HdsCrypto_Kdf_Derive_SKey(&kdf2, &Ecc2BC(hv), &sk2.get_Raw());
			verify_test(sk1 == sk2);
		}
	}
}
...
void TestSignature()
{
	...
	for (int i = 0; i < 5; i++)
	{
		...
		HdsCrypto_Signature sig2;
		HdsCrypto_Signature_Sign(&sig2, &Ecc2BC(msg), &sk.get_Raw());

		HdsCrypto_FlexPoint fp;
		fp.m_Compact = Ecc2BC(pk);
		fp.m_Flags = HdsCrypto_FlexPoint_Compact;

		verify_test(HdsCrypto_Signature_IsValid(&sig2, &Ecc2BC(msg), &fp));
...
void TestKrn()
{
	...
	for (int i = 0; i < 3; i++)
	{
		...
		verify_test(HdsCrypto_TxKernel_IsValid(&krn2));

		ECC::Hash::Value msg;
		HdsCrypto_TxKernel_getID(&krn2, &Ecc2BC(msg));
...
void TestPKdfExport()
{
	...
	for (int i = 0; i < 3; i++)
	{
		...
		HdsCrypto_KeyKeeper kk;
		HdsCrypto_Kdf_Init(&kk.m_MasterKey, &Ecc2BC(hv));
		...
		for (int j = 0; j < 3; j++)
		{
			...
			if (j)
			{
				...
				HdsCrypto_KeyKeeper_GetPKdf(&kk, &pkdf2, &iChild);
				hkdfChild.GenerateChild(hkdf, iChild);
			}
			else
			{
				pKdf1 = &hkdf;
				HdsCrypto_KeyKeeper_GetPKdf(&kk, &pkdf2, nullptr);
			}
...
{
	HdsCrypto_KeyKeeper m_Ctx;
	Key::IPKdf::Ptr m_pOwnerKey; // cached

	bool get_PKdf(Key::IPKdf::Ptr&, const uint32_t*);
	bool get_OwnerKey();

	static void CidCvt(HdsCrypto_CoinID&, const CoinID&);

	static void TxImport(HdsCrypto_TxCommon&, const Method::TxCommon&, std::vector<HdsCrypto_CoinID>&);
	static void TxExport(Method::TxCommon&, const HdsCrypto_TxCommon&);
...
{
	HdsCrypto_KdfPub pkdf;
	HdsCrypto_KeyKeeper_GetPKdf(&m_Ctx, &pkdf, pChild);
...
void KeyKeeperHwEmu::CidCvt(HdsCrypto_CoinID& cid2, const CoinID& cid)
{
	cid2.m_Idx = cid.m_Idx;
	cid2.m_SubIdx = cid.m_SubIdx;
	cid2.m_Type = cid.m_Type;
...
KeyKeeperHwEmu::Status::Type KeyKeeperHwEmu::InvokeSync(Method::CreateOutput& m)
{
	...
	if (!HdsCrypto_RangeProof_Calculate(&rp)) // Phase 2
		return Status::Unspecified;
...
{
	std::vector<HdsCrypto_CoinID> vCvt;
	HdsCrypto_TxCommon tx2;
	TxImport(tx2, m, vCvt);

	HdsCrypto_TxMutualInfo txMut;
	txMut.m_MyIDKey = m.m_MyIDKey;
	txMut.m_Peer = Ecc2BC(m.m_Peer);

	int nRet = HdsCrypto_KeyKeeper_SignTx_Receive(&m_Ctx, &tx2, &txMut);

	if (HdsCrypto_KeyKeeper_Status_Ok == nRet)
	{
		TxExport(m, tx2);
...
KeyKeeperHwEmu::Status::Type KeyKeeperHwEmu::InvokeSync(Method::SignSender& m)
{
	...
	HdsCrypto_TxSenderParams txSnd;
	txSnd.m_iSlot = m.m_Slot;
	txSnd.m_UserAgreement = Ecc2BC(m.m_UserAgreement);

	int nRet = HdsCrypto_KeyKeeper_SignTx_Send(&m_Ctx, &tx2, &txMut, &txSnd);
...
KeyKeeperHwEmu::Status::Type KeyKeeperHwEmu::InvokeSync(Method::SignSplit& m)
{
	...
	int nRet = HdsCrypto_KeyKeeper_SignTx_Split(&m_Ctx, &tx2);
...
void KeyKeeperHwEmu::TxImport(HdsCrypto_TxCommon& tx2, const Method::TxCommon& m, std::vector<HdsCrypto_CoinID>& v)
{
...
void KeyKeeperHwEmu::TxExport(Method::TxCommon& m, const HdsCrypto_TxCommon& tx2)
{
	// kernel
	assert(m.m_pKernel);
...
struct KeyKeeperWrap
{
		...
		:m_kkStd(get_KdfFromSeed(hv))
	{
		...
		HdsCrypto_Kdf_Init(&m_kkEmu.m_Ctx.m_MasterKey, &Ecc2BC(hv));
