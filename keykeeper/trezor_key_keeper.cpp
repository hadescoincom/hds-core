...
namespace hds::wallet
{
    ...
    namespace
    {
        ...
        void CidCvt(HdsCrypto_CoinID& cid2, const CoinID& cid)
        {
        ...
        struct Vectors
        {
            std::vector<HdsCrypto_CoinID> m_Inputs;
            std::vector<HdsCrypto_CoinID> m_Outputs;
        };

        void TxImport(HdsCrypto_TxCommon& tx2, const IPrivateKeyKeeper2::Method::TxCommon& m, Vectors& v)
        {
        ...
        void TxExport(HdsCrypto_CompactPoint& point, const HdsECCPoint& point2)
        {
            point.m_X = ConvertResultTo<HdsCrypto_UintBig>(point2.x());
            point.m_Y = static_cast<uint8_t>(point2.y());
        }

        void TxExport(HdsCrypto_Signature& signature, const HdsSignature& signature2)
        {
            signature.m_k = ConvertResultTo<HdsCrypto_UintBig>(signature2.sign_k());
            TxExport(signature.m_NoncePub, signature2.nonce_pub());
        }

        void TxExport(HdsCrypto_TxCommon& txCommon, const HdsTxCommon& tx2)
        {
            // kernel
            ...
            // offset
            txCommon.m_kOffset = ConvertResultTo<HdsCrypto_UintBig>(tx2.offset_sk());
        }

        void TxExport(IPrivateKeyKeeper2::Method::TxCommon& m, const HdsCrypto_TxCommon& tx2)
        {
            // kernel
            assert(m.m_pKernel);
    ...
    {
        m_DeviceManager->call_HdsGetPKdf(m.m_Root, m.m_iChild, true, [this, &m, h](const Message& msg, std::string session, size_t queue_size)
        {
            const HdsPKdf& hdsKdf = child_cast<Message, HdsPKdf>(msg);
            typedef struct
            {
            ...
            pkdf.m_Secret = ConvertResultTo<HdsCrypto_UintBig>(hdsKdf.key());
            
            TxExport(pkdf.m_CoFactorG, hdsKdf.cofactor_g());
            TxExport(pkdf.m_CoFactorJ, hdsKdf.cofactor_j());
    ...
    {
        m_DeviceManager->call_HdsGetNumSlots(true, [this, &m, h](const Message& msg, std::string session, size_t queue_size)
        {
            m.m_Count = child_cast<Message, HdsNumSlots>(msg).num_slots();
    ...
    void TrezorKeyKeeperProxy::CreateOutputCtx::Proceed3(Ptr& p)
    {
        ...
        m_This.m_DeviceManager->call_HdsGenerateRangeproof(&cid, &pt0, &pt1, nullptr, nullptr,
            [this, amp](const Message& msg, std::string session, size_t queue_size)
        {
            const HdsRangeproofData& brpd = child_cast<Message, HdsRangeproofData>(msg);
    ...
    void TrezorKeyKeeperProxy::InvokeAsync(Method::SignReceiver& m, const Handler::Ptr& h)
    {
        ...
        m_DeviceManager->call_HdsSignTransactionReceive(txCommon, txMutualInfo, [this, &m, h, txCommon](const Message& msg, std::string session, size_t queue_size) mutable
        {
            const auto& txReceive = child_cast<Message, HdsSignTransactionReceive>(msg);
            TxExport(txCommon, txReceive.tx_common());
            HdsCrypto_Signature proofSignature;
    ...
    void TrezorKeyKeeperProxy::InvokeAsync(Method::SignSender& m, const Handler::Ptr& h)
    {
        ...
        m_DeviceManager->call_HdsSignTransactionSend(txCommon, txMutualInfo, txSenderParams, [this, &m, h, txCommon](const Message& msg, std::string session, size_t queue_size) mutable
        {
            TxExport(txCommon, child_cast<Message, HdsSignTransactionSend>(msg).tx_common());
            HdsCrypto_UintBig userAgreement = ConvertResultTo<HdsCrypto_UintBig>(child_cast<Message, HdsSignTransactionSend>(msg).user_agreement());
    ...
    void TrezorKeyKeeperProxy::InvokeAsync(Method::SignSplit& m, const Handler::Ptr& h)
    {
        ...
        m_DeviceManager->call_HdsSignTransactionSplit(txCommon, [this, &m, h, txCommon](const Message& msg, std::string session, size_t queue_size) mutable
        {
            TxExport(txCommon, child_cast<Message, HdsSignTransactionSplit>(msg).tx_common());
            TxExport(m, txCommon);
            PushOut(Status::Success, h);
        });
    }
