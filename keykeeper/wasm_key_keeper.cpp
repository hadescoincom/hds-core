...
struct KeyKeeper
{
    ...
    std::string GetSendToken(const std::string& sbbsAddress, const std::string& identityStr, const std::string& amountStr)
    {
        ...
        hds::Amount amount = amountStrCopy.empty() ? 0 : std::stoull(amountStrCopy);
    ...
    static ECC::Key::IKdf::Ptr CreateKdfFromSeed(const std::string& phrase)
    {
        ...
        auto buf = hds::decodeMnemonic(string_helpers::split(phrase, ' '));
        ECC::Hash::Processor() << hds::Blob(buf.data(), (uint32_t)buf.size()) >> seed.V;
