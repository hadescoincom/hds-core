...
namespace
{
    ...
    {
        string jobID;
        hds::Merkle::Hash input;
        hds::Block::PoW pow;
        hds::IExternalPOW::BlockFound callback;
    };

    using SolutionCallback = function<void(Job&& job)>;

    class WorkProvider : public hdsMiner::minerBridge
    {
    public:
        ...
        void handleSolution(int64_t &workId, uint64_t &nonce, vector<uint32_t> &indices, uint32_t difficulty) override
        {
            ...
            hds::Block::PoW::NonceType t((const uint8_t*)&nonce);
            job.pow.m_Nonce = t;
            job.pow.m_Difficulty = hds::Difficulty(difficulty);
