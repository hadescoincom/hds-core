...
namespace hds
{
...
#ifdef WIN32
		if (m_pMapping)
            HDS_VERIFY(UnmapViewOfFile(m_pMapping));
		if (m_hMapping)
            HDS_VERIFY(CloseHandle(m_hMapping));
#else // WIN32
		if (m_pMapping)
            HDS_VERIFY(!munmap(m_pMapping, m_nMapping));
#endif // WIN32
	...
	void MappedFile::Close()
	{
		...
		if (INVALID_HANDLE_VALUE != m_hFile)
            HDS_VERIFY(CloseHandle(m_hFile));
#else // WIN32
		if (-1 != m_hFile)
            HDS_VERIFY(!close(m_hFile));
#endif // WIN32
