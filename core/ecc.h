...
namespace ECC
{
	...
	inline void GenRandom(hds::uintBig_t<nBytes_>& x) { GenRandom(x.m_pData, x.nBytes); }
	...
	typedef hds::uintBig_t<nBytes> uintBig;
...
#pragma pack (push, 1)
			struct Packed
			{
				hds::uintBigFor<uint64_t>::Type m_Idx;
				hds::uintBigFor<uint32_t>::Type m_Type;
				hds::uintBigFor<uint32_t>::Type m_SubIdx;
...
#pragma pack (pop)
	...
	namespace RangeProof
	{
		...
		struct CreatorParams
		{
			...
			Amount m_Value;
			hds::Blob m_Blob = hds::Blob(nullptr, 0); // max size is limited, together with m_Value should not exceed Scalar
