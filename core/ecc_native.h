...
namespace ECC
{
		...
		:private secp256k1_sha256_t
	{
		...
		void Write(const hds::Blob&);
		template <uint32_t nBytes_>
		void Write(const hds::uintBig_t<nBytes_>& x) { Write(x.m_pData, x.nBytes); }
		template <uint32_t n>
	...
	class NonceGenerator
	{
		...
		Hash::Value m_Prk;
		Hash::Value m_Okm;
		hds::uintBig_t<1> m_Counter; // wraps-around, it's fine
		bool m_bFirstTime;

		void Reset();
		void WriteIkm(const hds::Blob&);
