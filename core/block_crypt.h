...
namespace hds
{
	...
	struct Asset
	{
		typedef uint32_t ID; // 1-based asset index. 0 is reserved for default asset (Hds)
    ...
    struct Rules
    {
        ...
        struct {
            // emission parameters
            Amount Value0	= Coin * 190; // Initial emission. Each drop it will be halved. In case of odd num it's rounded to the lower value.
            Height Drop0	= 1440 * 180; // About half a year roughly. This is the height of the last block that still has the initial emission, the drop is starting from the next block
            Height Drop1	= 1440 * 180; // About half a year roughly. Each such a cycle there's a new drop
        } Emission;

        struct {
            Height Coinbase	= 240; // 4 hours. meaning that it will be available for spending 4 hours after it was mined.
        ...
        struct {
            ...
            Difficulty Difficulty0	= Difficulty(5 << Difficulty::s_MantissaBits); // 2^22 = 4,194,304. For GPUs producing 7 sol/sec this is roughly equivalent to 10K GPUs.
...
inline ECC::Hash::Processor& operator << (ECC::Hash::Processor& hp, const hds::PeerID& pid) {
	return hp << Cast::Down<ECC::Hash::Value>(pid);
}

// TODO: review this types, they don't have standard layout
inline void ZeroObject(hds::CoinID& x)
{
	ZeroObjectUnchecked(x);
}

inline void ZeroObject(hds::Block::SystemState::Full& x)
{
	ZeroObjectUnchecked(x);
}

inline void ZeroObject(hds::Asset::Full& x)
{
	ZeroObjectUnchecked(x);
}