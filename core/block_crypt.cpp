...
#undef THE_MACRO
	...
	void TxKernelShieldedInput::Sign(Lelantus::Prover& p, Asset::ID aid, bool bHideAssetAlways /* = false */)
	{
		...
		{
			// not necessary for hdss, just a demonstration of assets support
			m_pAsset = std::make_unique<Asset::Proof>();
...
#define COMPARE_TYPE(var, fnNext) \
 ...
 \
	...
	{
        TreasuryChecksum = {
            0x09, 0x0b, 0xb5, 0x31, 0xbb, 0x1c, 0x1d, 0xab,
            0xaa, 0x4f, 0xbf, 0x23, 0xed, 0x74, 0xdf, 0x37,
            0xb6, 0xbf, 0x6e, 0x43, 0x52, 0x32, 0xa8, 0x11,
            0x92, 0xe2, 0x13, 0x1b, 0x5a, 0x87, 0xa6, 0x52
        };

//        TreasuryChecksum = Zero;

		Prehistoric = {
            // BTC Block #16668 https://btc.tokenview.com/en/block/16668
            0x00, 0x00, 0x00, 0x00, 0x91, 0x4e, 0xcb, 0xb6,
            0xa9, 0xcf, 0x5f, 0x09, 0xa2, 0x56, 0xe8, 0x3a,
            0x0d, 0xda, 0x87, 0x12, 0x16, 0x79, 0x34, 0xce,
            0x75, 0x25, 0xcf, 0x05, 0xab, 0x2d, 0xdb, 0x1a,
		};
	...
	{
		return hds::Timestamp(std::chrono::seconds(std::time(nullptr)).count());
	}
