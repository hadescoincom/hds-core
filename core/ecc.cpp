...
//#	endif // __linux__
	...
	void Hash::Processor::Write(const hds::Blob& v)
	{
		Write(v.p, v.n);
	}
	...
	void NonceGenerator::WriteIkm(const hds::Blob& b)
	{
		assert(m_bFirstTime);
	...
	{
		NonceGenerator nonceGen1("hds-HKdf");
		nonceGen1 << hv;
	...
	{
		NonceGenerator nonceGen("hds-Schnorr");
	...
	namespace RangeProof
	{
		...
		{
			NonceGenerator nonceGen("hds-psig");
		...
		void Public::get_Msg(Hash::Value& hv, Oracle& oracle) const
		{
			oracle
				<< m_Value
				<< hds::Blob(&m_Recovery, sizeof(m_Recovery))
				>> hv;
		}
