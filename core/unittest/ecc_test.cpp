...
const hds::Height g_hFork = 3; // whatever
...
typedef hds::CoinID CoinID;

template <uint32_t nBytes>
void SetRandom(hds::uintBig_t<nBytes>& x)
{
	GenRandom(x);
}
...
void TestShifted(const hds::uintBig_t<n0>& x0, const hds::uintBig_t<n1>& x1, int nShift)
{
	TestShifted2(x0.m_pData, x0.nBytes, x1.m_pData, x1.nBytes, nShift);
}

template <uint32_t n0, uint32_t n1>
void TestShifts(const hds::uintBig_t<n0>& src, hds::uintBig_t<n0>& src2, hds::uintBig_t<n1>& trg, int nShift)
{
	src2 = src;
...
void TestUintBig()
{
	...
	{
		hds::uintBig_t<32> a;
		hds::uintBig_t<32 - 8> b;
		hds::uintBig_t<32 + 8> c;
		hds::uintBig_t<32> d;
...
void TestRangeProof(bool bCustomTag)
{
	...
	hds::uintBig_t<sizeof(Key::ID::Packed)> up0, up1;
	SetRandom(up0);
	cp.m_Blob = up0;
	...
	hds::uintBig_t<sizeof(Scalar) - sizeof(Amount) - 1> uc0, uc1;
	SetRandom(uc0);
	cp.m_Blob = uc0;
	...
	if (!bCustomTag) // coinbase with asset isn't allowed
	{
		...
		outp.Create(g_hFork, sk, kdf, cid, kdf, hds::Output::OpCode::Public);
	...
	WriteSizeSerialized("In-Utxo", hds::Input());
...
void TestMultiSigOutput()
{
    ...
    hds::Key::IKdf::Ptr pKdf_A;
    hds::Key::IKdf::Ptr pKdf_B;
    SetRandom(pKdf_B);
    SetRandom(pKdf_A);
	...
	hds::Output::GenerateSeedKid(cp.m_Seed.V, outp.m_Commitment, *pKdf_B);
    ...
    // create Input
    std::unique_ptr<hds::Input> pInput(new hds::Input);
    ...
    // output
    std::unique_ptr<hds::Output> pOutput(new hds::Output);
	*pOutput = outp;
    {
        Point::Native comm;
    ...
    std::unique_ptr<hds::TxKernelStd> pKernel(new hds::TxKernelStd);
    pKernel->m_Fee = 0;
    ...
    hds::TxBase::Context::Params pars;
    hds::TxBase::Context context(pars);
...
struct TransactionMaker
{
	...
	struct Peer
	{
		...
		void AddInput(hds::Transaction& t, Amount val, Key::IKdf& kdf, hds::Asset::ID nAssetID = 0)
		{
			std::unique_ptr<hds::Input> pInp(new hds::Input);
		...
		void AddOutput(hds::Transaction& t, Amount val, Key::IKdf& kdf, hds::Asset::ID nAssetID = 0)
		{
			std::unique_ptr<hds::Output> pOut(new hds::Output);
	...
	void CoSignKernel(hds::TxKernelStd& krn)
	{
	...
	void CreateTxKernel(std::vector<hds::TxKernel::Ptr>& lstTrg, Amount fee, std::vector<hds::TxKernel::Ptr>& lstNested, bool bEmitCustomTag, bool bNested)
	{
		std::unique_ptr<hds::TxKernelStd> pKrn(new hds::TxKernelStd);
		pKrn->m_Fee = fee;
		...
		// hashlock
		pKrn->m_pHashLock.reset(new hds::TxKernelStd::HashLock);
		//pKrn->m_pHashLock.release();

		hds::TxKernelStd::HashLock hl;
		...
		{
			// emit some asset
			hds::Asset::Metadata md;
			md.m_Value.assign({ 1, 2, 3, 4, 5 });
			md.UpdateHash();

			Scalar::Native sk;
			hds::Asset::ID nAssetID = 17;
			...
			hds::TxKernelAssetEmit::Ptr pKrnEmission(new hds::TxKernelAssetEmit);
...
void TestTransaction()
{
	...
	std::vector<hds::TxKernel::Ptr> lstNested, lstDummy;
	...
	hds::TxBase::Context::Params pars;
	hds::TxBase::Context ctx(pars);
	ctx.m_Height.m_Min = g_hFork;
	verify_test(tm.m_Trans.IsValid(ctx));
	verify_test(ctx.m_Stats.m_Fee == hds::AmountBig::Type(fee1 + fee2));
}
...
void TestCutThrough()
{
	...
	hds::Input::Ptr pInp(new hds::Input);
...
void TestBbs()
{
	...
	SetRandom(nonce);
	hds::ByteBuffer buf;
	verify_test(hds::proto::Bbs::Encrypt(buf, publicAddr, nonce, szMsg, sizeof(szMsg)));

	uint8_t* p = &buf.at(0);
	uint32_t n = (uint32_t) buf.size();

	verify_test(hds::proto::Bbs::Decrypt(p, n, privateAddr));
...
void TestRatio(const hds::Difficulty& d0, const hds::Difficulty& d1, double k)
{
...
void TestTreasury()
{
	...
	for (uint32_t i = 0; i < nPeers; i++)
	{
		...
		hds::PeerID pid;
		Scalar::Native sk;
		hds::Treasury::get_ID(pKdfs[i], pid, sk);

		// 2. Plan is created (2%, 3%, 4% of the total emission)
		hds::Treasury::Entry* pE = tres.CreatePlan(pid, hds::Rules::get().Emission.Value0 * (i + 2)/100, pars);
	...
	hds::Treasury::Data data;
	data.m_sCustomMsg = msg;
	tres.Build(data);
	verify_test(!data.m_vGroups.empty());

	std::vector<hds::Treasury::Data::Burst> vBursts = data.get_Bursts();
	...
	for (uint32_t i = 0; i < nPeers; i++)
	{
		std::vector<hds::Treasury::Data::Coin> vCoins;
...
void TestLelantus(bool bWithAsset)
{
	...
	hds::Lelantus::CmListVec lst;
	lst.m_vec.resize(N);

	Point::Native hGen;
	if (bWithAsset)
		hds::Asset::Base(35).get_Generator(hGen);
	...
	hds::Lelantus::Proof proof;
	proof.m_Cfg = cfg;
	hds::Lelantus::Prover p(lst, proof);

	hds::Sigma::Prover::UserData ud1, ud2;
	...
	Point pt_ = pt;
	Scalar::Native ser;
	hds::Lelantus::SpendKey::ToSerial(ser, pt_);
	...
	for (uint32_t iCycle = 0; iCycle < 3; iCycle++)
	{
		...
		hds::Executor::Scope scope(ex);

		uint32_t t = hds::GetTime_ms();
		...
		*PseudoRandomGenerator::s_pOverride = prg; // restore prnd state
		...
		if (iCycle)
		{
			...
			hds::SerializeBuffer sb = ser_.buffer();
...
void TestLelantusKeys()
{
	...
	hds::ShieldedTxo::PublicGen gen;
	gen.FromViewer(viewer);

	hds::ShieldedTxo::Data::TicketParams sprs, sprs2;
	hds::ShieldedTxo txo;
	...
	hds::ShieldedTxo::Data::OutputParams oprs, oprs2;
	oprs.m_Value = 3002U;
	oprs.m_AssetID = 18;
...
{
	const hds::Height hScheme = g_hFork;

	CoinID cidInpHds (170, 12, hds::Key::Type::Regular);
	CoinID cidInpAsset(53,  15, hds::Key::Type::Regular);
	CoinID cidOutHds (70,  25, hds::Key::Type::Regular);
	const hds::Amount fee = 100;

	hds::Key::IKdf::Ptr pKdf;
	HKdf::Create(pKdf, Zero);

	Scalar::Native sk, kOffset(Zero);

	hds::Asset::ID nAssetID = 24;
	hds::Asset::Metadata md;
	...
	hds::Input::Ptr pInp(new hds::Input);
	CoinID::Worker(cidInpHds).Create(sk, pInp->m_Commitment, *pKdf);
	tx.m_vInputs.push_back(std::move(pInp));
	kOffset += sk;

	hds::Input::Ptr pInpAsset(new hds::Input);
	CoinID::Worker(cidInpAsset).Create(sk, pInpAsset->m_Commitment, *pKdf);
	tx.m_vInputs.push_back(std::move(pInpAsset));
	kOffset += sk;

	hds::Output::Ptr pOutp(new hds::Output);
	pOutp->Create(hScheme, sk, *pKdf, cidOutHds, *pKdf);
	tx.m_vOutputs.push_back(std::move(pOutp));
	kOffset += -sk;

	hds::TxKernelStd::Ptr pKrn(new hds::TxKernelStd);
	pKdf->DeriveKey(sk, hds::Key::ID(23123, hds::Key::Type::Kernel));
	...
	hds::TxKernelAssetEmit::Ptr pKrnAsset(new hds::TxKernelAssetEmit);
	pKdf->DeriveKey(sk, hds::Key::ID(73123, hds::Key::Type::Kernel));
	pKrnAsset->m_AssetID = nAssetID;
	pKrnAsset->m_Value = -static_cast<hds::AmountSigned>(cidInpAsset.m_Value);
...
int main()
{
	...
	hds::Rules::get().CA.Enabled = true;
	hds::Rules::get().pForks[1].m_Height = g_hFork;
	hds::Rules::get().pForks[2].m_Height = g_hFork;
	ECC::TestAll();
	ECC::RunBenchmark();
