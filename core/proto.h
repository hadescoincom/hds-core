...
#define HdsNodeMsg_NewTip(macro) \
    macro(Block::SystemState::Full, Description)

#define HdsNodeMsg_GetHdr(macro) \
    macro(Block::SystemState::ID, ID)

#define HdsNodeMsg_Hdr(macro) \
    macro(Block::SystemState::Full, Description)

#define HdsNodeMsg_GetHdrPack(macro) \
    macro(Block::SystemState::ID, Top) \
    macro(uint32_t, Count)

#define HdsNodeMsg_HdrPack(macro) \
    macro(Block::SystemState::Sequence::Prefix, Prefix) \
    macro(std::vector<Block::SystemState::Sequence::Element>, vElements)

#define HdsNodeMsg_DataMissing(macro)

#define HdsNodeMsg_Status(macro) \
    macro(uint8_t, Value)

#define HdsNodeMsg_GetBody(macro) \
    macro(Block::SystemState::ID, ID)

#define HdsNodeMsg_GetBodyPack(macro) \
    macro(Block::SystemState::ID, Top) \
    macro(uint8_t, FlagP) \
    macro(uint8_t, FlagE) \
...
#define HdsNodeMsg_Body(macro) \
    macro(BodyBuffers, Body)

#define HdsNodeMsg_BodyPack(macro) \
    macro(std::vector<BodyBuffers>, Bodies)

#define HdsNodeMsg_GetProofState(macro) \
    macro(Height, Height)

#define HdsNodeMsg_GetCommonState(macro) \
    macro(std::vector<Block::SystemState::ID>, IDs)

#define HdsNodeMsg_GetProofKernel(macro) \
    macro(Merkle::Hash, ID)

#define HdsNodeMsg_GetProofKernel2(macro) \
    macro(Merkle::Hash, ID) \
    macro(bool, Fetch)

#define HdsNodeMsg_GetProofUtxo(macro) \
...
#define HdsNodeMsg_GetProofShieldedOutp(macro) \
    macro(ECC::Point, SerialPub)

#define HdsNodeMsg_GetProofShieldedInp(macro) \
    macro(ECC::Point, SpendPk)

#define HdsNodeMsg_GetProofAsset(macro) \
    macro(Asset::ID, AssetID) \
    macro(PeerID, Owner)

#define HdsNodeMsg_GetShieldedList(macro) \
    macro(TxoID, Id0) \
	macro(uint32_t, Count)

#define HdsNodeMsg_GetProofChainWork(macro) \
    macro(Difficulty::Raw, LowerBound)

#define HdsNodeMsg_ProofKernel(macro) \
    macro(TxKernel::LongProof, Proof)

#define HdsNodeMsg_ProofKernel2(macro) \
    macro(Merkle::Proof, Proof) \
    macro(Height, Height) \
    macro(TxKernel::Ptr, Kernel)

#define HdsNodeMsg_ProofUtxo(macro) \
    macro(std::vector<Input::Proof>, Proofs)

#define HdsNodeMsg_ProofShieldedOutp(macro) \
    macro(ECC::Point, Commitment) \
    macro(TxoID, ID) \
    macro(Height, Height) \
    macro(Merkle::Proof, Proof)

#define HdsNodeMsg_ProofShieldedInp(macro) \
    macro(Height, Height) \
    macro(Merkle::Proof, Proof)

#define HdsNodeMsg_ProofAsset(macro) \
    macro(Asset::Full, Info) \
    macro(Merkle::Proof, Proof)

#define HdsNodeMsg_ShieldedList(macro) \
    macro(TxoID, ShieldedOuts) \
    macro(std::vector<ECC::Point::Storage>, Items)

#define HdsNodeMsg_ProofState(macro) \
    macro(Merkle::HardProof, Proof)

#define HdsNodeMsg_ProofCommonState(macro) \
    macro(Block::SystemState::ID, ID) \
    macro(Merkle::HardProof, Proof)

#define HdsNodeMsg_ProofChainWork(macro) \
    macro(Block::ChainWorkProof, Proof)

#define HdsNodeMsg_Login0(macro) \
    macro(ECC::Hash::Value, CfgChecksum) \
    macro(uint8_t, Flags)

#define HdsNodeMsg_Login(macro) \
    macro(std::vector<ECC::Hash::Value>, Cfgs) \
    macro(uint32_t, Flags)

#define HdsNodeMsg_Ping(macro)
#define HdsNodeMsg_Pong(macro)

#define HdsNodeMsg_NewTransaction(macro) \
    macro(Transaction::Ptr, Transaction) \
    macro(bool, Fluff)

#define HdsNodeMsg_HaveTransaction(macro) \
    macro(Transaction::KeyType, ID)

#define HdsNodeMsg_GetTransaction(macro) \
    macro(Transaction::KeyType, ID)

#define HdsNodeMsg_Bye(macro) \
    macro(uint8_t, Reason)

#define HdsNodeMsg_PeerInfoSelf(macro) \
    macro(uint16_t, Port)

#define HdsNodeMsg_PeerInfo(macro) \
    macro(PeerID, ID) \
    macro(io::Address, LastAddr)

#define HdsNodeMsg_GetTime(macro)

#define HdsNodeMsg_Time(macro) \
    macro(Timestamp, Value)

#define HdsNodeMsg_GetExternalAddr(macro)

#define HdsNodeMsg_ExternalAddr(macro) \
    macro(uint32_t, Value)

#define HdsNodeMsg_BbsMsg(macro) \
    macro(BbsChannel, Channel) \
    macro(Timestamp, TimePosted) \
    macro(ByteBuffer, Message) \
    macro(Bbs::NonceType, Nonce)

#define HdsNodeMsg_BbsHaveMsg(macro) \
    macro(BbsMsgID, Key)

#define HdsNodeMsg_BbsGetMsg(macro) \
    macro(BbsMsgID, Key)

#define HdsNodeMsg_BbsSubscribe(macro) \
    macro(BbsChannel, Channel) \
    macro(Timestamp, TimeFrom) \
    macro(bool, On)

#define HdsNodeMsg_BbsResetSync(macro) \
    macro(Timestamp, TimeFrom)

#define HdsNodeMsg_SChannelInitiate(macro) \
    macro(PeerID, NoncePub)

#define HdsNodeMsg_SChannelReady(macro)

#define HdsNodeMsg_Authentication(macro) \
    macro(PeerID, ID) \
    macro(uint8_t, IDType) \
    macro(ECC::Signature, Sig)

#define HdsNodeMsg_GetEvents(macro) \
    macro(Height, HeightMin)

#define HdsNodeMsg_EventsLegacy(macro) \
    macro(std::vector<Event::Legacy>, Events)

#define HdsNodeMsg_Events(macro) \
    macro(ByteBuffer, Events)

#define HdsNodeMsg_EventsSerif(macro) \
    macro(ECC::Hash::Value, Value) \
    macro(Height, Height) \

#define HdsNodeMsg_GetBlockFinalization(macro) \
    macro(Height, Height) \
    macro(Amount, Fees)

#define HdsNodeMsg_BlockFinalization(macro) \
    macro(Transaction::Ptr, Value)

#define HdsNodeMsg_GetStateSummary(macro)

#define HdsNodeMsg_StateSummary(macro) \
...
#define HdsEventsAll(macro) \
        macro(1, Utxo) \
        macro(2, Shielded) \
        macro(3, AssetCtl)

#define HdsEvent_Utxo(macro) \
        macro(uint8_t, Flags) \
        macro(CoinID, Cid) \
        macro(ECC::Point, Commitment) \
        macro(Height, Maturity)

#define HdsEvent_Shielded(macro) \
        macro(uint8_t, Flags) \
        macro(TxoID, ID) \
        macro(Amount, Value) \
        macro(Asset::ID, AssetID) \
...
#define HdsEvent_AssetCtl(macro) \
        macro(uint8_t, Flags) \
...
#define THE_MACRO(id, name) \
 ...
 \
            HdsEvent_##name(THE_MACRO_DECL) \
 \
            template <typename Archive> \
            void serialize(Archive& ar) \
            { \
                HdsEvent_##name(THE_MACRO_SER) \
            } \
        };
...
#define THE_MACRO1(code, msg) \
    ...
    { \
        ...
        HdsNodeMsg_##msg(THE_MACRO2) \
        template <typename Archive> void serialize(Archive& ar) { ar HdsNodeMsg_##msg(THE_MACRO3); } \
        msg(Zero_ = Zero) { HdsNodeMsg_##msg(THE_MACRO4) } /* default c'tor, zero-init everything */ \
        msg(Uninitialized_) { } /* don't init members */ \
        msg(HdsNodeMsg_##msg(THE_MACRO5) Unused_ = Unused) { HdsNodeMsg_##msg(THE_MACRO6) } /* explicit init */ \
    }; \
    struct msg##_NoInit :public msg { \
        msg##_NoInit() :msg(Uninitialized) {} \
    }; \

    HdsNodeMsgsAll(THE_MACRO1)
#undef THE_MACRO1
#undef THE_MACRO2
#undef THE_MACRO3
#undef THE_MACRO4
