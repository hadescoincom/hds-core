...
void hdsStratum::getWork(int64_t* workOut, uint64_t* nonceOut, uint8_t* dataOut, uint32_t*) {
	*workOut = workId;
...
void hdsStratum::testAndSubmit(int64_t wId, uint64_t nonceIn, vector<uint32_t> indices) {
...
void hdsStratum::handleSolution(int64_t &workIdVar, uint64_t &nonceVar, vector<uint32_t> &indices, uint32_t) {
	std::thread (&hdsStratum::testAndSubmit,this, workIdVar, nonceVar,indices).detach();
}


hdsStratum::hdsStratum(string hostIn, string portIn, string apiKeyIn, bool debugIn) : res(io_service), context(boost::asio::ssl::context::sslv23)  {
	host = hostIn;
	port = portIn;
	apiKey = apiKeyIn;
	debug = debugIn;
