...
/********
    ...
    Hds Hash III Verify Functions & CPU Miner
...
bool HdsHash_III::IsValidSolution(const blake2b_state& base_state, std::vector<uint8_t> soln) {
...
SolverCancelledException hdsSolverCancelled;

bool HdsHash_III::OptimisedSolve(const blake2b_state& base_state,
	...
	for (uint32_t i=0; i<(1 << (collisionBitSize+1)); i++) {
		...
		if (cancelled(ListGeneration)) throw hdsSolverCancelled;
	}
	...
	for (round=1; round<5; round++) {
		...
		for (uint32_t i=0; i<elements.size(); i++) {
			...
			if (cancelled(MixElements)) throw hdsSolverCancelled;
		}

		// Sorting
		std::sort(elements.begin(), elements.end(), sortStepElement);
		if (cancelled(ListSorting)) throw hdsSolverCancelled;
		...
		for (uint32_t i=0; i<elements.size()-1; i++) {
			...
			if (cancelled(ListColliding)) throw hdsSolverCancelled;
		}
