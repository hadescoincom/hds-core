...
#if defined(__ANDROID__) || !defined(HDS_USE_AVX)
...
inline constexpr size_t hdshash_solution_size(unsigned int N, unsigned int K) {
...
static EquihashR<150,5,0> HdsHashI;
static EquihashR<150,5,3> HdsHashII;


#define EhRInitialiseState(n, k, r, base_state)  \
     if (n == 150 && k == 5 && r == 0) { 				\
        HdsHashI.InitialiseState(base_state); 	\
    } else if (n == 150 && k == 5 && r == 3)) {       			\
        HdsHashII.InitialiseState(base_state);	\
    } else {                                 				\
...
#define EhRIsValidSolution(n, k, r, base_state, soln, ret)   	\
    if (n == 150 && k == 5 && r == 0) {                 	\
        ret = HdsHashI.IsValidSolution(base_state, soln); 	\
    } else if (n == 150 && k == 5 && r == 3)) {            	\
        ret = HdsHashII.IsValidSolution(base_state, soln);  	\
    } else {                                             	\
        throw std::invalid_argument("Unsupported Equihash parameters"); \
    }
