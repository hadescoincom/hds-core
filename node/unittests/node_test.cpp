	...
	void TestNodeClientProto()
	{
			...
			:public proto::NodeConnection
		{
			...
			virtual void OnMsg(proto::ShieldedList&& msg) override
			{
				...
				pKrn->Sign(p, 0, true); // hide asset, although it's hds
		...
		node.GenerateRecoveryInfo(hds::g_sz3);

		struct MyParser
			:public hds::RecoveryInfo::IRecognizer
		{
			uint32_t m_Spent = 0;
			uint32_t m_Utxos = 0;
			uint32_t m_Assets = 0;
		...
		p.Proceed(hds::g_sz3); // check we can rebuild the Live consistently with shielded and assets

		verify_test((p.m_SpendKeys.size() == 1) && (p.m_Spent == 1) && p.m_Utxos && p.m_Assets);

		auto logger = hds::Logger::create(LOG_LEVEL_DEBUG, LOG_LEVEL_DEBUG);
		node.PrintTxos();
...
void TestAll()
{
	...
	//auto logger = hds::Logger::create(LOG_LEVEL_DEBUG, LOG_LEVEL_DEBUG);
	if (!bClientProtoOnly)
		hds::PrintEmissionSchedule();

	hds::Rules::get().AllowPublicUtxos = true;
	hds::Rules::get().FakePoW = true;
	hds::Rules::get().MaxRollback = 10;
	hds::Rules::get().DA.WindowWork = 35;
	hds::Rules::get().Maturity.Coinbase = 35; // lowered to see more txs
	hds::Rules::get().Emission.Drop0 = 5;
	hds::Rules::get().Emission.Drop1 = 8;
	hds::Rules::get().CA.Enabled = true;
	hds::Rules::get().Maturity.Coinbase = 10;
	hds::Rules::get().pForks[1].m_Height = 16;
	hds::Rules::get().UpdateChecksum();
	...
	verify_test(hds::helpers::ProcessWideLock("/tmp/HDS_node_test_lock"));

	hds::DeleteFile(hds::g_sz);
	hds::DeleteFile(hds::g_sz2);
	...
	if (!bClientProtoOnly)
	{
...

		{
			...
			std::vector<hds::BlockPlus::Ptr> blockChain;
			hds::TestNodeProcessor1(blockChain);
			hds::DeleteFile(hds::g_sz);
			hds::DeleteFile(hds::g_sz2);
			...
			hds::TestNodeProcessor2(blockChain);
			hds::DeleteFile(hds::g_sz);

			printf("NodeProcessor test3...\n");
			fflush(stdout);

			hds::TestNodeProcessor3(blockChain);
			hds::DeleteFile(hds::g_sz);
			hds::DeleteFile(hds::g_sz2);
		}
		...
		hds::TestNodeConversation();
		hds::DeleteFile(hds::g_sz);
		hds::DeleteFile(hds::g_sz2);
	}

	hds::Rules::get().pForks[2].m_Height = 17;
	hds::Rules::get().CA.DepositForList = hds::Rules::Coin * 16;
	hds::Rules::get().Shielded.m_ProofMax = { 4, 6 }; // 4K
	hds::Rules::get().Shielded.m_ProofMin = { 4, 5 }; // 1K
	hds::Rules::get().UpdateChecksum();

	printf("Node <---> Client test (with proofs)...\n");
	fflush(stdout);

	hds::TestNodeClientProto();

	{
		// test utxo set image rebuilding with shielded in/outs
		hds::io::Reactor::Ptr pReactor(hds::io::Reactor::create());
		hds::io::Reactor::Scope scope(*pReactor);

		std::string sPath;
		hds::NodeProcessor::get_UtxoMappingPath(sPath, hds::g_sz);
		hds::DeleteFile(sPath.c_str());

		hds::Node node;
		node.m_Cfg.m_sPathLocal = hds::g_sz;
		node.Initialize();
	}

	hds::DeleteFile(hds::g_sz);
	hds::DeleteFile(hds::g_sz2);
	hds::DeleteFile(hds::g_sz3);
