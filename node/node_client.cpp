...
namespace
{
    ...
    {
#ifdef WIN32
        boost::filesystem::path boostPath{ hds::Utf8toUtf16(path.c_str()) };
#else
    ...
    void removeNodeDataIfNeeded(const std::string& nodePathStr)
    {
        try
        {
            ...
            catch (const hds::NodeDBUpgradeException&)
            {
            }
...
void NodeClient::setKdf(hds::Key::IKdf::Ptr kdf)
{
    m_pKdf = kdf;
}

void NodeClient::setOwnerKey(hds::Key::IPKdf::Ptr key)
{
    m_ownerKey = key;
}
