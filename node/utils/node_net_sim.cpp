...
bool ReadSeed(Key::IKdf::Ptr& pKdf, const char* szSeed)
{
    ...
    if (vWords.size() != hds::WORD_COUNT)
        return false;

    hds::ByteBuffer buf = hds::decodeMnemonic(vWords);

    ECC::Hash::Value hvSeed;
    ECC::Hash::Processor() << hds::Blob(buf) >> hvSeed;
...
#undef THE_MACRO
    ...
    bool SendShieldedInp(TxoSH& txo)
    {
        ...
        {
            hds::Executor::Scope scope(m_Exec);
            pKrn->Sign(p, 0, true);
        };
...
int main_Guarded(int argc, char* argv[])
{
    ...
    auto logger = hds::Logger::create(LOG_LEVEL_INFO, LOG_LEVEL_INFO);
...
#undef THE_MACRO
    ...
    else
    {
        ...
        uint64_t nIndex = 1;
        HDS_VERIFY(pE->m_pResponse->Create(pE->m_Request, *pKdf, nIndex));
        ...
        hds::DeleteFile(node.m_Cfg.m_sPathLocal.c_str());
    }
