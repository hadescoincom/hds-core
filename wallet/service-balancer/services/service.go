...
func newService(cfg *Config, index int, port int, logname string) (svc *service, err error) {
	...
	svc.context      = ctxWS
	svc.command      = exec.CommandContext(ctxWS, cfg.ServiceExePath, "-n", cfg.HdsNodeAddress, "-p", strconv.Itoa(port))
	...
	log.Printf("%v %v, starting as [%v %v %v %v%v]", logname, index, cfg.ServiceExePath, "-n " + cfg.HdsNodeAddress, "-p", port, cliOptions)
