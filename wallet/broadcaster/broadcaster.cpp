...
bool parseWalletUpdateInfo(const std::string& versionString, const std::string& typeString, WalletImplVerInfo& walletVersionInfo)
{
    ...
    switch (appType)
    {
        case VersionInfo::Application::DesktopWallet:
            walletVersionInfo.m_message = "Hds Wallet UI " + versionString;
            break;
...
int main_impl(int argc, char* argv[])
{
    ...
    auto logger = hds::Logger::create(LOG_LEVEL_DEBUG, LOG_LEVEL_DEBUG, LOG_LEVEL_DEBUG, "broadcast_", path.string());
    ...
    try
    {
...

        {
            ...
            messageDesc.add_options()
                ...
                (cli::EXCHANGE_CURR, po::value<std::string>(&options.exchangeRate.currency), "currency: 'hds', 'btc', 'ltc', 'qtum'")
