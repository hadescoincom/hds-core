...
namespace
{
    ...
    {
        SecString pass;
        if (!hds::read_wallet_pass(pass, vm))
        {
    ...
    int InitDataBase(const po::variables_map& vm, bool generateNewSeed)
    {
        ...
        if (vm.count(cli::PASS) == 0 && !hds::confirm_wallet_pass(pass))
        {
    ...
    void AddVoucherParameter(const po::variables_map& vm, TxParameters& params, IWalletDB::Ptr db, uint64_t ownID)
    {
        if (auto it = vm.find(cli::VOUCHER_COUNT); it != vm.end())
        {
            ...
            if (!vouchers.empty())
            {
                ...
                params.SetParameter(TxParameterID::TransactionType, hds::wallet::TxType::PushTransaction);
            }
        }
    }
    ...
    int GetToken(const po::variables_map& vm)
    {
        ...
        {
            params.SetParameter(TxParameterID::TransactionType, hds::wallet::TxType::Simple);
        }
    ...
    void ShowAssetTxs(const po::variables_map& vm, const IWalletDB::Ptr& walletDB, Asset::ID assetId)
    {
        ...
        {
            // there cannot be 'orphaned' asset push/pull transactions but hds only, so skip in this case
            if (assetId != Asset::s_InvalidID)
            {
    ...
    void ShowHDSInfo(const po::variables_map& vm)
    {
        auto walletDB = OpenDataBase(vm);
        Block::SystemState::ID stateID = {};
        walletDB->getSystemStateID(stateID);
        ...
        if (vm.count(cli::SWAP_TX_HISTORY))
        {
            ...
            else
            {
                ...
                for (auto& tx : txHistory) {
                    ...
                    storage::getTxParameter(*walletDB, tx.m_txId, wallet::kDefaultSubTxID,
                        wallet::TxParameterID::AtomicSwapIsHdsSide, isHdsSide);
                    ...
                    stringstream ss;
                    ss << (isHdsSide ? kHDS : to_string(swapCoin)) << " <--> "
                        << (!isHdsSide ? kHDS : to_string(swapCoin));
    ...
    int ShowWalletInfo(const po::variables_map& vm)
    {
        ...
        // Otherwise we display both HDS & ASSETS
        //
    ...
    int ExportPaymentProof(const po::variables_map& vm)
    {
        ...
        if (!res.empty())
        {
            ...
            hds::to_hex(&sTxt.front(), res.data(), res.size());
    ...
    bool LoadReceiverParams(const po::variables_map& vm, TxParameters& params)
    {
        ...
        if (auto peerID = params.GetParameter<WalletID>(hds::wallet::TxParameterID::PeerID); peerID && std::to_string(*peerID) != addressOrToken)
        {
            params.SetParameter(hds::wallet::TxParameterID::OriginalToken, addressOrToken);
        }
        return true;
    }
    ...
    bool LoadBaseParamsForTX(const po::variables_map& vm, Asset::ID& assetId, Amount& amount, Amount& fee, WalletID& receiverWalletID, bool checkFee, bool skipReceiverWalletID=false)
    {
        ...
        if(vm.count(cli::ASSET_ID)) // asset id can be zero if hds only
        {
    ...
    boost::optional<TxID> InitSwap(const po::variables_map& vm, const IWalletDB::Ptr& walletDB, Wallet& wallet, bool checkFee)
    {
        ...
        bool isHdsSide = (vm.count(cli::SWAP_HDS_SIDE) != 0);
    ...
    boost::optional<TxID> AcceptSwap(const po::variables_map& vm, const IWalletDB::Ptr& walletDB, Wallet& wallet, bool checkFee)
    {
        ...
        auto swapTxParameters = hds::wallet::ParseParameters(swapTxToken);

        // validate TxType and parameters
        auto transactionType = swapTxParameters->GetParameter<TxType>(TxParameterID::TransactionType);
        auto isHdsSide = swapTxParameters->GetParameter<bool>(TxParameterID::AtomicSwapIsHdsSide);
        auto swapCoin = swapTxParameters->GetParameter<AtomicSwapCoin>(TxParameterID::AtomicSwapCoin);
        auto hdsAmount = swapTxParameters->GetParameter<Amount>(TxParameterID::Amount);
        ...
        bool isValidToken = isHdsSide && swapCoin && hdsAmount && swapAmount && peerID && peerResponseTime && createTime && minHeight;
...
#ifdef HDS_LIB_VERSION
        if (auto libVersion = swapTxParameters->GetParameter(hds::wallet::TxParameterID::LibraryVersion); libVersion)
        {
        ...
        cout << " Swap conditions: " << "\n"
            ...
            << " Hds amount:  " << PrintableAmount(*hdsAmount) << "\n"
        ...
        FillSwapFee(&(*swapTxParameters), fee, swapFeeRate, *isHdsSide);
...
#ifdef HDS_CONFIDENTIAL_ASSETS_SUPPORT
           if (Rules::get().CA.Enabled && withAssets)
            {
                RegisterAssetCreators(wallet);
            }
#endif  // HDS_CONFIDENTIAL_ASSETS_SUPPORT
...
#ifdef HDS_LASER_SUPPORT
        {cli::LASER,                HandleLaser,                    "laser hds command"},
#endif  // HDS_LASER_SUPPORT
...
#define LOG_FILES_PREFIX "wallet_"
        ...
        auto logger = hds::Logger::create(logLevel, logLevel, fileLogLevel, LOG_FILES_PREFIX, path.string());
