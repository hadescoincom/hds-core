...
namespace hds::wallet
{
    ...
    void AssetIssueTxBuilder::SelectInputCoins()
    {
        ...
        if (m_Tx.GetParameter(TxParameterID::PreselectedCoins, preselIDs, m_SubTxID))
        {
            if (!preselIDs.empty())
            {
                ...
                for (auto &coin : coins)
                {
                    isAssetCoin(coin) ? preselAmountAsset += coin.getAmount() : preselAmountHds += coin.getAmount();
        ...
        Amount amountHdsWithFee = m_Fee; // for issue/consue HDSs are only used for fees
        if (preselAmountHds < amountHdsWithFee)
        {
            ...
            if (selectedCoins.empty())
            {
                ...
                LOG_ERROR() << m_Tx.GetTxID() << "[" << m_SubTxID << "]" << " You only have " << PrintableAmount(hdsTotals.Avail);
        ...
        {
            coin.m_spentTxId = m_Tx.GetTxID();
            isAssetCoin(coin) ? totalAsset += coin.m_ID.m_Value : totalHds += coin.m_ID.m_Value;
    ...
    void AssetIssueTxBuilder::GenerateHdsCoin(Amount amount, bool change)
    {
        ...
        m_OutputCoins.push_back(newUtxo.m_ID);
        m_Tx.SetParameter(TxParameterID::OutputCoins, m_OutputCoins, false, m_SubTxID);
        LOG_INFO() << m_Tx.GetTxID() << " Creating HDS coin" << (change ? " (change):" : ":")
