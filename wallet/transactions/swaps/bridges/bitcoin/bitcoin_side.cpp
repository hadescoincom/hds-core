...
namespace
{
    ...
    libbitcoin::chain::script CreateAtomicSwapContract(const hds::wallet::BaseTransaction& tx, bool isBtcOwner, uint8_t addressVersion)
    {
        using namespace hds;
        ...
        if (NoLeak<uintBig> secretPrivateKey; tx.GetParameter(TxParameterID::AtomicSwapSecretPrivateKey, secretPrivateKey.V, SubTxIndex::HDS_REDEEM_TX))
        {
        ...
        {
            Point publicKeyPoint = tx.GetMandatoryParameter<Point>(TxParameterID::AtomicSwapSecretPublicKey, SubTxIndex::HDS_REDEEM_TX);
...
{
    BitcoinSide::BitcoinSide(BaseTransaction& tx, bitcoin::IBridge::Ptr bitcoinBridge, bitcoin::ISettingsProvider& settingsProvider, bool isHdsSide)
        : m_tx(tx)
        , m_bitcoinBridge(bitcoinBridge)
        , m_settingsProvider(settingsProvider)
        , m_isBtcOwner(!isHdsSide)
    {
    ...
    bool BitcoinSide::ValidateLockTime()
    {
        ...
        double blocksPerHdsBlock = GetBlocksPerHour() / hds::Rules::get().DA.Target_s;
        Height hdsCurrentHeight = m_tx.GetWalletDB()->getCurrentHeight();
        Height hdsHeightDiff = hdsCurrentHeight - m_tx.GetMandatoryParameter<Height>(TxParameterID::MinHeight, SubTxIndex::HDS_LOCK_TX);

        Height peerMinHeight = externalLockTime - GetLockTimeInBlocks();
        Height peerEstCurrentHeight = peerMinHeight + static_cast<Height>(std::ceil(blocksPerHdsBlock * hdsHeightDiff));
    ...
    bool BitcoinSide::HasEnoughTimeToProcessLockTx()
    {
        ...
        if (m_tx.GetParameter(TxParameterID::MaxHeight, lockTxMaxHeight, SubTxIndex::HDS_LOCK_TX))
        {
            Block::SystemState::Full systemState;
            if (m_tx.GetTip(systemState) && systemState.m_Height > lockTxMaxHeight - GetLockTxEstimatedTimeInHdsBlocks())
            {
                return false;
            }
        }
    ...
    uint32_t BitcoinSide::GetLockTxEstimatedTimeInHdsBlocks() const
    {
        // it's average value
        return 70;
    }
    ...
    void BitcoinSide::InitSecret()
    {
        ...
        m_tx.SetParameter(TxParameterID::AtomicSwapSecretPrivateKey, secretPrivateKey.V, false, HDS_REDEEM_TX);
    }
    ...
    void BitcoinSide::FillWithdrawTx(SubTxID subTxID)
    {
        try
        {
            ...
            if (SubTxIndex::REFUND_TX == subTxID && quickRefund)
                localPrivateKey.V = m_tx.GetMandatoryParameter<uintBig>(hds::wallet::TxParameterID::AtomicSwapPeerPrivateKey);
            else
                localPrivateKey.V = m_tx.GetMandatoryParameter<uintBig>(hds::wallet::TxParameterID::AtomicSwapPrivateKey);
            ...
            else
            {
                ...
                NoLeak<uintBig> secretPrivateKey;
                m_tx.GetParameter(TxParameterID::AtomicSwapSecretPrivateKey, secretPrivateKey.V, SubTxIndex::HDS_REDEEM_TX);
                libbitcoin::ec_secret secret;
                std::copy(std::begin(secretPrivateKey.V.m_pData), std::end(secretPrivateKey.V.m_pData), secret.begin());
