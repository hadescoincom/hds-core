...
void TestAssets() {
    ...
    hds::io::Reactor::Ptr mainReactor{hds::io::Reactor::create()};
    hds::io::Reactor::Scope scope(*mainReactor);
    ...
    hds::wallet::RegisterAssetCreators(sender.m_Wallet);

    //const Key::Index assetOwnerIdx = Key::Index(22);

    Asset::ID assetId = 445; // whatever

    const auto checkTotals = [&] (Amount hds, Amount amountAsset) {
        storage::Totals allTotals(*walletDB);
        WALLET_CHECK(allTotals.GetHdsTotals().Avail == AmountBig::Type(hds));
    ...
    const auto initialAmount      = hds::Amount(700);
    const auto issueAmount        = hds::Amount(500);
    const auto feeAmount          = hds::Amount(100);
    ...
    WALLET_CHECK(issueTx.m_changeHds  == issueChange);
    ...
    WALLET_CHECK(consumeTx.m_changeHds  == consumeChange);
    ...
    // Hds change coin generated while issuing asset, we spend it during consume
    ...
    // This is hds returned for consumed assets
    ...
    const auto selfSendHdsChange  = consumeAmount - feeAmount;
    ...
    WALLET_CHECK(selfSendTx.m_changeHds  == selfSendHdsChange);
    ...
    // This is hds returned for consumed assets before, we've used it to pay for self send transaction
    ...
    checkTotals(selfSendHdsChange, selfSendAmount);
    checkTotals(100, 300);
    ...
    WALLET_CHECK(srConsumeTx.m_changeHds    == srHdsChange);
    ...
    // This is hds generated from consumed asset
    ...
    const auto send3rpHdsChange  = srConsumeAmount - feeAmount;
    ...
    WALLET_CHECK(send3rpTx.m_changeHds   == send3rpHdsChange);
    ...
    WALLET_CHECK(send3rpRTx.m_changeHds   == 0);
    ...
    WALLET_CHECK(send3rpCoins[8].getAmount()  == feeAmount + send3rpHdsChange);
    ...
    WALLET_CHECK(send3rpCoins[9].getAmount()  == send3rpHdsChange);
    WALLET_CHECK(send3rpCoins[9].m_createTxId == send3rpTx.m_txId);

    checkTotals(send3rpHdsChange, 0);
    checkTotals(100, 0);
    ...
    const auto nonOwnedHdsChange    = 0;
    ...
    WALLET_CHECK(nonOwnedConsumeTx.m_changeHds    == nonOwnedHdsChange);
    ...
    const auto recv3rpHdsChange = Amount(0);
    ...
    WALLET_CHECK(recv3rpTx.m_changeHds   == recv3rpHdsChange);
    ...
    WALLET_CHECK(recv3rpRTx.m_changeHds   == 0);
    ...
    checkTotals(send3rpHdsChange, recv3rpAmount);
    checkTotals(100, 100);
    ...
    WALLET_CHECK(recvConsumeTx.m_changeHds    == recvHdsChange);
...
int main () {
    ...
    const auto logger = hds::Logger::create(logLevel, logLevel);
    LOG_INFO() << "Assets test - starting";

    auto& rules = hds::Rules::get();
