...
namespace
{
    ...
    TxParameters AcceptSwapParameters(const TxParameters& initialParameters, const WalletID& myID, Amount fee, Amount feeRate)
    {
        ...
        bool isHdsSide = !*parameters.GetParameter<bool>(TxParameterID::AtomicSwapIsHdsSide);

        if (isHdsSide)
        {
            // delete parameters from other side
            parameters.DeleteParameter(TxParameterID::Fee, SubTxIndex::HDS_REDEEM_TX);
            ...
            // add our parameters
            parameters.SetParameter(TxParameterID::Fee, fee, SubTxIndex::HDS_LOCK_TX);
            parameters.SetParameter(TxParameterID::Fee, fee, SubTxIndex::HDS_REFUND_TX);
            parameters.SetParameter(TxParameterID::Fee, feeRate, SubTxIndex::REDEEM_TX);
        }
        else
        {
            // delete parameters from other side
            parameters.DeleteParameter(TxParameterID::Fee, SubTxIndex::HDS_LOCK_TX);
            parameters.DeleteParameter(TxParameterID::Fee, SubTxIndex::HDS_REFUND_TX);
            parameters.DeleteParameter(TxParameterID::Fee, SubTxIndex::REDEEM_TX);

            // add our parameters
            parameters.SetParameter(TxParameterID::Fee, fee, SubTxIndex::HDS_REDEEM_TX);
        ...
        parameters.SetParameter(TxParameterID::IsSender, isHdsSide);
        parameters.SetParameter(TxParameterID::AtomicSwapIsHdsSide, isHdsSide);
...
void TestSwapTransaction(bool isHdsOwnerStart, hds::Height fork1Height, bool useSecureIDs = false)
{
    ...
    NodeObserver observer([&]()
    {
        ...
        if (cursor.m_Sid.m_Height == fork1Height + 5)
        {
            ...
            bool isHdsSide = !isHdsOwnerStart;
            auto parameters = InitNewSwap(isHdsOwnerStart ? receiver.m_WalletID : sender.m_WalletID,
                currentHeight, hdsAmount, hdsFee, wallet::AtomicSwapCoin::Bitcoin, swapAmount, feeRate, isHdsSide);

            if (useSecureIDs)
            {
                parameters.SetParameter(TxParameterID::MyWalletIdentity, isHdsOwnerStart ? receiver.m_SecureWalletID : sender.m_SecureWalletID);
            }
            ...
            auto acceptParams = AcceptSwapParameters(parameters, acceptor->m_WalletID, hdsFee, feeRate);
            if (useSecureIDs)
            {
    ...
    WALLET_CHECK(receiverCoins[0].m_ID.m_Value == hdsAmount - hdsFee);
    ...
    // change
    WALLET_CHECK(senderCoins[4].m_ID.m_Value == 500 - hdsAmount - hdsFee);
    ...
    storage::getTxParameter(*sender.m_WalletDB, txID, SubTxIndex::HDS_REDEEM_TX, TxParameterID::AtomicSwapSecretPrivateKey, senderSecretPrivateKey.V);
    NoLeak<uintBig> receiverSecretPrivateKey;
    storage::getTxParameter(*receiver.m_WalletDB, txID, SubTxIndex::HDS_REDEEM_TX, TxParameterID::AtomicSwapSecretPrivateKey, receiverSecretPrivateKey.V);
    WALLET_CHECK(senderSecretPrivateKey.V != Zero && senderSecretPrivateKey.V == receiverSecretPrivateKey.V);
}

void TestElectrumSwapTransaction(bool isHdsOwnerStart, hds::Height fork1Height)
{
    ...
    NodeObserver observer([&]()
        {
            ...
            if (cursor.m_Sid.m_Height == fork1Height + 5)
            {
                ...
                if (isHdsOwnerStart)
                {
                    ...
                    txID = sender.m_Wallet.StartTransaction(AcceptSwapParameters(parameters, sender.m_WalletID, hdsFee, feeRate));
                }
                else
                {
                    sender.m_Wallet.StartTransaction(parameters);
                    txID = receiver.m_Wallet.StartTransaction(AcceptSwapParameters(parameters, receiver.m_WalletID, hdsFee, feeRate));
                }
            }
        });
...
void TestSwapTransactionWithoutChange(bool isHdsOwnerStart)
{
    cout << "\nTesting atomic swap transaction...\n";
    ...
    if (isHdsOwnerStart)
    {
        auto parameters = InitNewSwap(receiver.m_WalletID, currentHeight, hdsAmount, hdsFee, wallet::AtomicSwapCoin::Bitcoin, swapAmount, feeRate, false);
        receiver.m_Wallet.StartTransaction(parameters);
        txID = sender.m_Wallet.StartTransaction(AcceptSwapParameters(parameters, sender.m_WalletID, hdsFee, feeRate));
    }
    else
    {
        auto parameters = InitNewSwap(sender.m_WalletID, currentHeight, hdsAmount, hdsFee, wallet::AtomicSwapCoin::Bitcoin, swapAmount, feeRate, true);
        sender.m_Wallet.StartTransaction(parameters);
        txID = receiver.m_Wallet.StartTransaction(AcceptSwapParameters(parameters, receiver.m_WalletID, hdsFee, feeRate));
    }
...
void TestSwapBTCRefundTransaction()
{
    ...
    auto parameters = InitNewSwap(receiver->m_WalletID, currentHeight, hdsAmount, hdsFee, wallet::AtomicSwapCoin::Bitcoin, swapAmount, feeRate, false);
    receiver->m_Wallet.StartTransaction(parameters);
    TxID txID = sender->m_Wallet.StartTransaction(AcceptSwapParameters(parameters, sender->m_WalletID, hdsFee, feeRate));
...
void TestSwapHdsRefundTransaction()
{
    cout << "\nAtomic swap: testing Hds refund transaction...\n";
    ...
    eventToUpdate = io::AsyncEvent::create(*mainReactor, [&sender, &receiver, txID, &eventToUpdate, &node]()
    {
        if (receiver)
        {
            wallet::AtomicSwapTransaction::State txState = wallet::AtomicSwapTransaction::State::Initial;
            storage::getTxParameter(*receiver->m_WalletDB, txID, wallet::kDefaultSubTxID, wallet::TxParameterID::State, txState);
            if (txState == wallet::AtomicSwapTransaction::State::SendingHdsRedeemTX)
            {
                // delete receiver to simulate refund on Hds side
                receiver.reset();
            }
            eventToUpdate->post();
        }
        else
        {
            wallet::AtomicSwapTransaction::State txState = wallet::AtomicSwapTransaction::State::Initial;
            storage::getTxParameter(*sender->m_WalletDB, txID, wallet::kDefaultSubTxID, wallet::TxParameterID::State, txState);
            if (txState != wallet::AtomicSwapTransaction::State::SendingHdsRefundTX)
            {
                // speed-up test
                node.AddBlock();
                eventToUpdate->post();
    ...
    // change of Hds LockTx
    WALLET_CHECK(senderCoins[4].m_ID.m_Value == 500 - hdsAmount - hdsFee);
    WALLET_CHECK(senderCoins[4].m_status == Coin::Available);
    WALLET_CHECK(senderCoins[4].m_createTxId == txID);

    // Refund
    WALLET_CHECK(senderCoins[5].m_ID.m_Value == hdsAmount - hdsFee);
    WALLET_CHECK(senderCoins[5].m_status == Coin::Available);
    WALLET_CHECK(senderCoins[5].m_createTxId == txID);
}

void TestSwapHdsAndBTCRefundTransaction()
{
    cout << "\nAtomic swap: testing Hds and BTC refund transactions...\n";
    ...
    NodeObserver observer([&]()
    {
        ...
        if (cursor.m_Sid.m_Height == minHeight)
        {
            ...
            auto parameters = InitNewSwap(receiver->m_WalletID, minHeight, hdsAmount, hdsFee, wallet::AtomicSwapCoin::Bitcoin, swapAmount, feeRate, false);
            receiver->m_Wallet.StartTransaction(parameters);
            txID = sender->m_Wallet.StartTransaction(AcceptSwapParameters(parameters, sender->m_WalletID, hdsFee, feeRate));
            auto receiverCoins = receiver->GetCoins();
            WALLET_CHECK(receiverCoins.empty());
...
{
    cout << "\nAtomic swap: testing BTC redeem after Hds expired...\n";
...
void TestElectrumSwapHdsRefundTransaction()
{
    cout << "\nAtomic swap: testing Hds refund transaction on electrum...\n";
...
void ExpireByResponseTime(bool isHdsSide)
{
    // Simulate swap transaction without response from second side

    cout << "\nAtomic swap: testing expired transaction on " << (isHdsSide ? "Hds" : "BTC") << " side...\n";
    ...
    TestNode node{ TestNode::NewBlockFunc(), kNodeStartHeight };
    Height currentHeight = node.m_Blockchain.m_mcm.m_vStates.size();
    auto swapParameters = InitNewSwap(receiverWalletID, currentHeight, hdsAmount, hdsFee, wallet::AtomicSwapCoin::Bitcoin, swapAmount, feeRate, !isHdsSide, lifetime, responseTime);
    TxID txID = sender->m_Wallet.StartTransaction(AcceptSwapParameters(swapParameters, sender->m_WalletID, hdsFee, feeRate));
    io::Timer::Ptr timer = io::Timer::create(*mainReactor);
    ...
    eventToUpdate = io::AsyncEvent::create(*mainReactor, [&]()
        {
            if (receiver)
            {
                ...
                if (txState == wallet::AtomicSwapTransaction::State::BuildingHdsLockTX && isNeedReset)
                {
                    receiver.reset();
                    isNeedReset = false;
                }
...
void TestIgnoringThirdPeer()
{
    ...
    eventToUpdate = io::AsyncEvent::create(*mainReactor, [&]()
    {
        ...
        if (result)
        {
            wallet::AtomicSwapTransaction::State txState = wallet::AtomicSwapTransaction::State::Initial;
            storage::getTxParameter(*sender->m_WalletDB, txID, wallet::kDefaultSubTxID, wallet::TxParameterID::State, txState);
            if (txState == wallet::AtomicSwapTransaction::State::BuildingHdsLockTX)
            {
                // create new address
                ...
                SetTxParameter msg;
                msg.AddParameter(TxParameterID::SubTxIndex, SubTxIndex::HDS_REFUND_TX)
...
int main()
{
    ...
    auto logger = hds::Logger::create(logLevel, logLevel);
    Rules::get().FakePoW = true;
    Rules::get().UpdateChecksum();
    hds::Height fork1Height = 10;
    ...
    TestSwapHdsRefundTransaction();
    TestSwapHdsAndBTCRefundTransaction();
    ...
    TestSwapCancelTransaction(true, wallet::AtomicSwapTransaction::State::BuildingHdsLockTX);
    TestSwapCancelTransaction(false, wallet::AtomicSwapTransaction::State::BuildingHdsLockTX);

    TestSwapCancelTransaction(true, wallet::AtomicSwapTransaction::State::BuildingHdsRedeemTX);
    TestSwapCancelTransaction(false, wallet::AtomicSwapTransaction::State::BuildingHdsRedeemTX);

    TestSwapCancelTransaction(true, wallet::AtomicSwapTransaction::State::BuildingHdsRefundTX);
    TestSwapCancelTransaction(false, wallet::AtomicSwapTransaction::State::BuildingHdsRefundTX);
    ...
    TestElectrumSwapHdsRefundTransaction();
#endif // HDS_TESTNET
