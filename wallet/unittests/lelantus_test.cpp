...
void TestShieldedUTXORollback()
{
    ...
    auto coins = db->getShieldedCoins(Asset::s_HdsID);
    WALLET_CHECK(coins.size() == 1);
...
void TestPushTxRollbackByLowFee()
{
    ...
    auto shieldedCoins = sender.m_WalletDB->getShieldedCoins(Asset::s_HdsID);
...
void TestPullTxRollbackByLowFee()
{
    ...
    WALLET_CHECK(coins.size() == 0);
    auto shieldedCoins = sender.m_WalletDB->getShieldedCoins(Asset::Asset::s_HdsID);
