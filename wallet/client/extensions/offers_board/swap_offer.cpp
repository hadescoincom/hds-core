...
bool SwapOffer::IsValid() const
{
    ...
    auto isHdsSide = GetParameter<bool>(TxParameterID::AtomicSwapIsHdsSide);
    ...
    return type && *type == TxType::AtomicSwap && swapCoin && isHdsSide
        && amount && swapAmount && responseTime && minimalHeight && txId;
}

bool SwapOffer::isHdsSide() const
{
    bool res = true;
    GetParameter(TxParameterID::AtomicSwapIsHdsSide, res);
    return res;
}

Amount SwapOffer::amountHds() const
{
    Amount amount = 0;
...
Height SwapOffer::peerResponseHeight() const
{
    ...
    GetParameter(hds::wallet::TxParameterID::PeerResponseTime,
                 peerResponseHeight);
...
Height SwapOffer::minHeight() const
{
    ...
    GetParameter(hds::wallet::TxParameterID::MinHeight, minHeight);
    return minHeight;
}
