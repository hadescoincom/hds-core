...
namespace hds::wallet
{
    ...
    void BaseTxBuilder::SelectFeeInputsPreferUnlinked()
    {
        ...
        if (m_Tx.GetParameter(TxParameterID::PreselectedCoins, preselectedCoinIDs, m_SubTxID) && !preselectedCoinIDs.empty())
        {
            ...
            for (auto& coin : coins)
            {
                if (!coin.isAsset())
                {
                    preselAmountHds += coin.getAmount();
        ...
        const Amount amountHds = GetFee();
        if (preselAmountHds < amountHds)
        {
            auto selectedCoins = m_Tx.GetWalletDB()->selectUnlinkedCoins(amountHds, Zero);
            if (selectedCoins.empty())
            {
                selectedCoins = m_Tx.GetWalletDB()->selectCoins(amountHds, Zero);
            }
        ...
        m_ChangeHds  = totalHds  - amountHds;
        m_ChangeAsset = 0;
    ...
    void BaseTxBuilder::SelectInputs()
    {
        ...
        const bool isHdsTransaction = !IsAssetTx();
        const Amount amountHdsWithFee = (isHdsTransaction ? GetAmount() : 0) + GetFee();
        if (preselAmountHds < amountHdsWithFee)
        {
            auto selectedCoins = m_Tx.GetWalletDB()->selectCoins(amountHdsWithFee - preselAmountHds, Zero);
            copy(selectedCoins.begin(), selectedCoins.end(), back_inserter(coins));
            if (selectedCoins.empty())
            {
                storage::Totals allTotals(*m_Tx.GetWalletDB());
        ...
        const Amount amountAsset = isHdsTransaction ? 0 : GetAmount();
        ...
        for (auto& coin : coins)
        {
            ...
            else totalHds += coin.m_ID.m_Value;
            m_InputCoins.push_back(coin.m_ID);
        }

        m_ChangeHds  = totalHds  - amountHdsWithFee;
    ...
    void BaseTxBuilder::GenerateHdsCoin(Amount amount, bool change)
    {
